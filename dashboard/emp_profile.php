        ...
        <form method="POST">
          <?php
            if(isset($_SESSION['danger'])) {
              $msg = $_SESSION['danger'];
              echo "<p class='p-2 text-danger text-center'>".$msg."</p>";
...
<?php 
  ...
  if(isset($_POST['work_btn'])) {
    ...
    $query = mysqli_query($config,$check);
    $rows = mysqli_num_rows($query);
    if($rows) {
      $_SESSION['danger'] = "<small>You can't submit work again in same date :)</small>";
      ...
      if($query2) {
        $_SESSION['success'] = "<small>Work submited successfully :)</small>";
        header('location: emp_profile.php');
      }
      else {
        $_SESSION['danger'] = "<small>Work doesn't submited, Please try again :)</small>";
        header('location: emp_profile.php');
      }
    }
  }
