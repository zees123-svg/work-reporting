<?php
  ...
  if(isset($_POST['add_emp'])) {
    $fullname = mysqli_real_escape_string($config,$_POST['user_name']);
    $des = mysqli_real_escape_string($config,$_POST['user_des']);
    $res = mysqli_real_escape_string($config,$_POST['user_res']);
    $scale = mysqli_real_escape_string($config,$_POST['user_scale']);
    $user_id = mysqli_real_escape_string($config,$_POST['user_id']);
    $pass = $_POST['user_pass'];
    $hashed_pass = password_hash($pass, PASSWORD_DEFAULT);
    $role = mysqli_real_escape_string($config,$_POST['user_role']);
    if(strlen($pass) < 4 ) {
      $_SESSION['error'] = "<small>Password must be at least 4 character long</small>";
      ...
      if ($rows) {
        $_SESSION['error'] = "<small>User ID already exist</small>";
        header('location: add_emp.php'); 
        exit;
      }
      else {
        $insert = "INSERT INTO user_tbl (fullname,user_des,user_scale,user_res,user_id,user_pass,user_role) VALUES ('$fullname','$des','$scale','$res','$user_id','$hashed_pass','$role')";
        $query1 = mysqli_query($config,$insert);
        if($query1) {
